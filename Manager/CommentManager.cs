...
namespace Manager
{
    public class CommentManager
    {
        ...
        public List<CommentDt> GetPage(string pageindex, int pagesize, out int pagecount)
        {
            int p;
            int rowcount;
            FormatVerify.PageCheck(pageindex, out p);
            List<CommentDt> list = server.GetPage(p, pagesize, out rowcount);
            pagecount = (int)Math.Ceiling(rowcount * 1.0 / pagesize);
            PhotoManager manager=new PhotoManager();
            foreach (CommentDt item in list)
            {
                item.PhotoImgUrl = manager.Get(item.PhotoId).ImgUrl;
            }
            return list;
        }
    }
